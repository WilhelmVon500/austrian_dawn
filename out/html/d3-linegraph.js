...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    ...
    if (!partyColors) {
        partyColors = {'oclk': '#EEEE22', 'ap': '#FA5A42', 'ovp': '#000000', 'vf': '#ED2939', 'dnlp': '#0E428E', 'other': '#909090'};,
    }
    if (!partyNames) {
        partyNames = {'oclk': Q.oclk_name, 'ap': 'AP', 'ovp': 'ÖVP', 'vf': 'VF', 'dnlp': 'DNLP', 'other': 'Others'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
    ...
    function linegraph(dataset) {
     dataset.each(function (data) {
      ...
      const xScale = d3.scaleUtc([new Date(1941, 0), addMonths(maxDate, additionalMonths)], [marginLeft, width - marginRight]);
      ...
      if (!dataMax) {
          const maxVF = d3.max(data, d => d.vf);
          const maxOCLK = d3.max(data, d => d.oclk);
          dataMax = maxVF >= maxOCLK ? maxVF + 10 : maxOCLK + 10;
          dataMin = 0;
      }
